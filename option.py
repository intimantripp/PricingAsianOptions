...
class Option:
    ...
    def set_bottom_boundary(self):
        for i in range(self.numt + 1):
            self.grid[0, i] = self.initial_value_at_bottom(i)

    def set_top_boundary(self):
        for i in range(self.numt + 1):
            self.grid[self.numx, i] = self.initial_value_at_top(i)

    def set_initial_boundary(self):
        for j in range(self.numx + 1):
            self.grid[j, 0] = self.initial_value_at_height(j)
